...
% تنش‌ها
\node[tension] (t1) at (-2.5, 0.4) {\rl{فرد}\\% FIXED: گره چندخطی
  \rl{در برابر جمع}%
  };
\node[tension] (t2) at ( 2.5, 0.4) {\rl{صوری}\\%
  \rl{در برابر واقعی}%
  };
\node[tension] (t3) at (0, 1.5)    {\rl{اقتدار}\\%
  \rl{در برابر آزادی}%
  };
\node[tension] (t4) at (-2.5,-1.2) {\rl{هویت}\\%
  \rl{در برابر انتخاب}%
  };
\node[tension] (t5) at ( 2.5,-1.2) {\rl{مسئولیت}\\%
  \rl{در برابر ساختار}%
  };
